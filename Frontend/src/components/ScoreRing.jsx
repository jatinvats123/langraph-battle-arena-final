...
export default function ScoreRing({ score = 0, max = 10, size = 86, isWinner = false }) {
  ...
  const strokeColor = isWinner ? (isDark ? '#c8c8c8' : '#C96443') : (isDark ? '#555555' : '#d5d5d5');
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} viewBox="0 0 90 90" style={{ transform: 'rotate(-90deg)' }}>
        {/* Track */}
        <circle cx="45" cy="45" r={radius} fill="none" stroke={isDark ? "rgba(255,255,255,0.07)" : "#e5e5e5"} strokeWidth="5" />
        {/* Progress */}
        <circle
          ref={circleRef}
      ...
      <div style={{
        ...
        <span style={{
          ...
          lineHeight: 1,
          color: isWinner ? (isDark ? '#e8e8e8' : '#333') : (isDark ? '#888' : '#999'),
        }}>
          {score}
        </span>
        <span style={{
